    ...
    if (uiAcUiState.currentDevice == null) {
        ...
        LazyColumn(
            ...
            item {
                ...
                var checked = uiAcUiState.currentDevice?.status == Status.ON
                if (checked) {
                    Text(
                ...
                Spacer(modifier = Modifier.height(6.dp))
                var sliderPosition = uiAcUiState.currentDevice?.temperature ?: 0
                Slider(
                    value = sliderPosition.toFloat(),
                    onValueChange = {
                        sliderPosition = it.roundToInt(); acViewModel.setTemperature(
                        sliderPosition
                    )
                    },
                    steps = 19,
                ...
                Spacer(modifier = Modifier.height(6.dp))
                var selectedIndex =uiAcUiState.currentDevice?.mode?.ordinal ?: 0
                val options = listOf(
                    stringResource(id = R.string.ac_mode_cool),
                    stringResource(id = R.string.ac_mode_heat),
                    stringResource(id = R.string.ac_mode_fan)
                )
                SingleChoiceSegmentedButtonRow {
                    options.forEachIndexed { index, label ->
                        SegmentedButton(
                            ...
                            onClick = {
                                selectedIndex = index; acViewModel.setMode(AcMode.entries[index])
                            },
                ...
                var selectedOption1 =acViewModel.uiState.value.currentDevice?.horizontalSwing ?: options1[0]
                ...
                // State to track the currently selected option
                var selectedOption2 =acViewModel.uiState.value.currentDevice?.verticalSwing ?: options2[0] // Default to "Auto"
                ...
                // State to track the currently selected option
                var selectedOption3 =acViewModel.uiState.value.currentDevice?.fanSpeed ?: options3[0]
